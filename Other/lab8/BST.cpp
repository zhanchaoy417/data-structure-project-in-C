...
bool isValidBSTHelp(Node* node,float min,float max)
{
    if(node==NULL)
    {
        return true;
    ...
    bool isLeftBST=isValidBSTHelp(node->left,min,node->key);
    bool isrightBST=isValidBSTHelp(node->right,node->key,max);
...
bool BST::isValidBST()
{
  ...
  isValidBSTHelp(root,(getMinValueNode(root))->key,(getMaxValueNode(root))->key);
